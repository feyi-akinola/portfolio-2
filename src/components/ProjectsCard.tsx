...
export default function ProjectsCard({style} : ProjectsCardProps) {
  return (
    <div className="flex flex-col h-full w-full gap-2" style={{ gridRow: "span 2", ...style }}>
      <div
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row lg:flex-col gap-8 lg:gap-0">
          <div
            className="w-full sm:w-[50%] lg:w-full h-full min-h-40 rounded-xl mb-4 overflow-hidden flex items-center justify-center bg-black/70"
          >
          </div>
          ...
          <div className="w-full sm:w-[50%] lg:w-full mt-12 flex flex-col">
            {PROJECTS.map((p, _, __) => (
              <div
                key={p.name}
                className="mb-4 flex justify-center align-center py-3.5 text-base lg:text-lg font-medium uppercase cursor-pointer text-text"
              >
                <div className="flex relative">
                  <p className="button-text_ text-[0.85em]">{p.name}</p>
                  {
                    p.type === "Design" &&
                      <span className="absolute -top-2 -right-12 flex items-center text-[0.5rem] px-2 py-1 bg-text rounded-full text-white ">
                        {p.type}
                      </span>
                  }
                </div>
      ...
      <div className="pt-4 bg-stone-100 rounded-2xl px-6 py-5 flex items-center justify-around gap-2">
        {SOCIALS.map(({ href, icon, label }) => (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="flex items-center gap-1.5 text-xs font-semibold text-text no-underline transition-opacity duration-200 hover:opacity-60"
          >
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center bg-black text-base`}>
              {icon ?? "📄"}
            </div>
            {label}
          </a>
